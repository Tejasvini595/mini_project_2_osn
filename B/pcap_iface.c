...
// **NEW**: Struct to pass arguments to the capture thread
typedef struct {
    char *device;
    char *filter_exp;
} capture_args_t;
...
void print_payload(const u_char *payload, int len) {
    ...
    for (int i = 0; i < max_bytes; i += bytes_per_line) {
        ...
        for (int j = 0; j < bytes_per_line; j++)
            if (i + j < max_bytes) printf("%02x ", payload[i + j]);
            else printf("   ");
        printf(" ");
        for (int j = 0; j < bytes_per_line; j++)
            if (i + j < max_bytes)
...
void handle_tcp(const u_char *packet, int size, int total_ip_len) {
    if (size < sizeof(struct tcphdr)) { printf("L4 (TCP): Malformed Packet\n"); return; }
    const struct tcphdr *tcp_header = (const struct tcphdr *)packet;
    uint16_t src_port = ntohs(tcp_header->th_sport), dst_port = ntohs(tcp_header->th_dport);
    const char *src_svc = get_service_name(src_port), *dst_svc = get_service_name(dst_port);
    int hdr_len = tcp_header->th_off * 4;
    printf("L4 (TCP): Src Port: %u%s%s | Dst Port: %u%s%s | Seq: %u | Ack: %u\n",
           src_port, src_svc ? " (" : "", src_svc ? src_svc : "",
           dst_port, dst_svc ? ")" : "", dst_svc ? dst_svc : "",
    ...
    printf("L7 (Payload): Identified as %s on port %u - %d bytes\n",
           app_proto, dst_svc ? dst_port : src_port, payload_len);
    print_payload(payload, payload_len);
}

void handle_udp(const u_char *packet, int size) {
    if (size < sizeof(struct udphdr)) { printf("L4 (UDP): Malformed Packet\n"); return; }
    const struct udphdr *udp_header = (const struct udphdr *)packet;
    uint16_t src_port = ntohs(udp_header->uh_sport), dst_port = ntohs(udp_header->uh_dport);
    const char *src_svc = get_service_name(src_port), *dst_svc = get_service_name(dst_port);
...
void handle_ipv4(const u_char *packet, int size) {
    if (size < sizeof(struct ip)) { printf("L3 (IPv4): Malformed Packet\n"); return; }
    ...
    if (ip_hdr->ip_p == IPPROTO_TCP) handle_tcp(transport_packet, transport_size, total_ip_len);
    else if (ip_hdr->ip_p == IPPROTO_UDP) handle_udp(transport_packet, transport_size);
}

void handle_ipv6(const u_char *packet, int size) {
    // Basic IPv6 handler, can be expanded
    if (size < sizeof(struct ip6_hdr)) { printf("L3 (IPv6): Malformed Packet\n"); return; }
...
void handle_arp(const u_char *packet, int size) {
    if (size < sizeof(struct arphdr)) { printf("L3 (ARP): Malformed Packet\n"); return; }
...
void packet_handler(u_char *user, const struct pcap_pkthdr *h, const u_char *bytes) {
    ...
    if (h->caplen < sizeof(struct ether_header)) { printf("L2 (Ethernet): Malformed Packet\n"); return; }
    ...
    switch (type) {
        case ETHERTYPE_IP: handle_ipv4(next, remaining); break;
        case ETHERTYPE_IPV6: handle_ipv6(next, remaining); break;
        case ETHERTYPE_ARP: handle_arp(next, remaining); break;
    }
    fflush(stdout);
}
...
// **MODIFIED**: Capture thread now accepts a struct with device and filter
static void *capture_thread_fn(void *arg) {
    ...
    if (!handle) {
        ...
        capture_running = 0; return NULL;
    }

    // **NEW**: Apply the filter if one was provided
    if (args->filter_exp) {
        struct bpf_program fp;
        bpf_u_int32 netmask;
        bpf_u_int32 ip;

        // Find the IP and netmask of the device
        ...
        if (pcap_compile(handle, &fp, args->filter_exp, 0, netmask) == -1) {
            ...
            pcap_close(handle); capture_running = 0; return NULL;
        }
        if (pcap_setfilter(handle, &fp) == -1) {
            fprintf(stderr, "Error setting filter '%s': %s\n", args->filter_exp, pcap_geterr(handle));
            pcap_freecode(&fp); pcap_close(handle); capture_running = 0; return NULL;
        }
        pcap_freecode(&fp);
    }
...
// **NEW**: Central function to start any capture session
void start_capture_session(char *device, char *filter_exp) {
    ...
    if (pthread_create(&cap_thread, NULL, capture_thread_fn, args) != 0) {
        ...
        free(args->device); free(args->filter_exp); free(args);
        return;
    }

    // Main thread waits for user to press Ctrl+C or Ctrl+D
    ...
    while (capture_running && !exit_requested) {
        if (getline(&line, &len, stdin) == -1) {
            ...
            else { perror("getline"); exit_requested = 1; }
            capture_running = 0;
        }
    }
    free(line);
...
// **NEW**: Function to handle the filter selection menu
...
static char **list_devices_and_get_array(int *out_count) {
    ...
    if (count == 0) { pcap_freealldevs(alldevs); *out_count = 0; return NULL; }
...
int main(void) {
    ...
    while (1) {
        ...
        if (getline(&line, &len, stdin) == -1) goto cleanup_and_exit;
        long v = strtol(line, NULL, 10);
        if (v > 0 && v <= devcount) { chosen_index = v - 1; break; }
    ...
    while (running && !exit_requested) {
        ...
        printf("  3. Inspect Last Session (not implemented)\n");
        ...
        switch (atoi(line)) {
            case 1: start_capture_session(chosen_device, NULL); break;
            case 2: show_filter_menu_and_capture(chosen_device); break;
            case 3: printf("Option not implemented yet.\n"); break;
            case 4: running = 0; break;
            default: printf("Unknown choice. Try again.\n"); break;
        }
    }
