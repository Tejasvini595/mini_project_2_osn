...
found:
  ...
  // BONUS: Initialize LFU-Recent algorithm fields
  ...
  for(int i = 0; i < MAX_RESIDENT_PAGES; i++) {
    ...
    // BONUS: Initialize LFU-Recent fields
...
freeproc(struct proc *p)
{
  ...
  p->trapframe = 0;
  if(p->pagetable) {
    // Clean up resident pages first
    free_resident_pages(p->pagetable, p);
    proc_freepagetable(p->pagetable, p->sz);
  }
  p->pagetable = 0;
  
  //part1 - Cleanup demand paging structures
  // Mark all page tracking entries as unused
  for(int i = 0; i < MAX_RESIDENT_PAGES; i++) 
  {
    p->resident_pages[i].in_use = 0;
  }
  ...
  // Close executable file reference
  if(p->exec_inode) 
  {
    iput(p->exec_inode);
    p->exec_inode = 0;
  ...
  // TODO: Delete swap file when implemented
...
kfork(void)
{
  ...
  //part2 - Copy demand paging metadata from parent to child
  ...
  // Note: Don't copy resident_pages or swapped_pages - child gets its own
...
kexit(int status)
{
  ...
  //part3 - Clean up swap file before process exits
  cleanup_swap_file();
