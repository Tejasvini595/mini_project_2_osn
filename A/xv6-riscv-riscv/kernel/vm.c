...
vmfault(pagetable_t pagetable, uint64 va, int is_write)
{
  ...
  // Check if in stack region (between heap end and stack_top)
  // We allow access to the entire allocated stack space
  else if(page_va >= p->sz && page_va < p->stack_top) {
    is_valid = 1;
    cause = "stack";
  ...
  if(is_swapped(page_va)) 
  {
    ...
    if((page_va >= p->heap_start && page_va < p->sz) || 
       (page_va >= p->sz && page_va < p->stack_top)) {
...
allocate_zero_page(pagetable_t pagetable, uint64 va, int is_write)
{
  ...
  // Map the page with appropriate permissions
  int perm = PTE_R | PTE_U;
  // Heap pages (between heap_start and sz) and stack pages (between sz and stack_top) are writable
  if((va >= p->heap_start && va < p->sz) || (va >= p->sz && va < p->stack_top)) {
    perm |= PTE_W; // Heap and stack are writable
  }
  
  printf("[pid %d] ALLOCATE_ZERO_PAGE: va=0x%lx perm=0x%x (heap: 0x%lx-0x%lx, stack: 0x%lx-0x%lx)\n", 
         p->pid, va, perm, p->heap_start, p->sz, p->sz, p->stack_top);
  ...
  if(is_dirty) 
  {
    ...
    if(pa != 0) 
    {
      ...
      if(slot < 0) 
      {
        ...
        printf("[pid %d] Process terminated: swap full\n", p->pid);
        setkilled(p);
        return 0;
      }
